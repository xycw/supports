...
?>
    <tr<?php if($entry['state']!=1) echo ' class="bg-info"'?>>
        ...
        <td>
            ...
            if($entry['state']==1){
                $title = '标记未读';
                $link = U('Customers/Contact/mark', array('site_id'=>$entry['site_id'], 'contact_id'=>$entry['contact_us_records_id'], 'state'=>0));
            }else{
                $title = '标记已读';
                $link = U('Customers/Contact/mark', array('site_id'=>$entry['site_id'], 'contact_id'=>$entry['contact_us_records_id'], 'state'=>1));
            }
            ?>
...
<div class="page-nav">
	<div class="row">
		...
		<?php 
		W('Common/PageNavigation/page', array('page'=>$page,'num'=>$num,'count'=>$count, 'name'=>'Customers/Contact/list', $page_data));
		?>
		</div>
	</div>
</div>
