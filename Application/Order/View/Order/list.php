...
if(I('order_status_remark')!=''){
    ...
    echo '<p class="bg-primary">当前筛选订单状态：';

    foreach ($order_status_remark_select as $status){
                ...
                <tr class="order-hd<?php if(risk_order($entry)) echo ' risk-order'?>">
                    ...
                    <td<?php if((($comments=hasComment($entry['site_id'], $entry['orders_id']))!==false)) echo ' style="background-color:yellow;"';?>  width="150px">                        
                        ...
                        <a class="link-view f08" <?php if($comments!==false) echo 'data-toggle="tooltip" data-placement="top" data-html="true" title="'.htmlspecialchars ($comments).'"'?> id="link-view-<?php echo $entry['site_id'] . '-' . $entry['orders_id'] ?>" href="<?php echo U('Order/Order/view', array('site_id' => $entry['site_id'], 'order_id' => $entry['orders_id'])) ?>">
							...
							if(empty($entry['order_no'])) $order_no =  $entry['order_no_prefix'] . $entry['orders_id'];
							echo $order_no;
                            ?></a>
                    ...
                    <td>
						...
						echo $entry['order_status_remark'] == '' ? '待处理' : (session(C('USER_INFO').'.profile_id') == 6 ? $entry['order_status_remark'] : '<a class="email_template_dialog" href="' . U('Order/Order/email/site_id/' . $entry['site_id'] . '/order_id/' . $entry['orders_id']) . '">' . $entry['order_status_remark'] . '</a>') . '<span class="badge" data-toggle="tooltip" data-placement="bottom" data-html="true" title="' . $tip . '">' . $num_email . '</span>';
                    ...
                    <td>
<?php
if($entry['orders_products_remark_count']){
    $yidinghuo = true;
    foreach($entry['orders_products_remark_count'] as $entry_orders_products_remark_count){
?>
    <?php
                ...
                <div class="page-nav-info">
                   ...
                   <label>订单数量:</label>
                    ...
                    <?php
                        ...
                        $page_num_data = array(1=>1,25=>25,50=>50,100=>100, 200=>200,300=>300, 500=>500);
            ...
            <div class="modal-body">
                ...
                <form action="<?php echo U('Order/Order/payment_confirmation/action/upload') ?>" enctype="multipart/form-data" method="post">
                    ...
                    <div class="form-group">
                      ...
                      <p class="help-block">请按照<a href="__PUBLIC__/example/stripe_payment.csv" target="__blank">stripe_payment.csv</a>标准格式导入!</p>
