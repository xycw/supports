...
?>
    <tr class="order-hd" id="tr<?php echo $product['product_id'].'_'.$products_detail['language_code'] ?>">
       ...
       <?php	
        ...
        <?php
            if(!empty($product['get_model'])){
                echo '<br/><span style="font-size:12px;color:#ff0000"><b>采集SKU: </b>'.$product['get_model'].'</span>';
            }
        ?>
        ...
        //1-NBA现货，2-NFL,3-冰球NHL,4-MLB,5-NCAA,6-足球服,7-T卫衣杂款,8-帽子,9-定制款,10-待归类
        	switch ($product['orders_products_categories_id']) {
        		case '1':
        			$orders_products_categories_name='NBA现货';
        			break;
        		case '2':
        			$orders_products_categories_name='NFL';
        			break;
        		case '3':
        			$orders_products_categories_name='冰球NHL';
        			break;
        		case '4':
        			$orders_products_categories_name='MLB';
        			break;
        		case '5':
        			$orders_products_categories_name='NCAA';
        			break;
        		case '6':
        			$orders_products_categories_name='足球服';
        			break;
        		case '7':
        			$orders_products_categories_name='T卫衣杂款';
        			break;
        		case '8':
        			$orders_products_categories_name='帽子';
        			break;
        		case '9':
        			$orders_products_categories_name='定制款';
        			break;
        		case '10':
        			$orders_products_categories_name='待归类';
        			break;
        		default:
        			$orders_products_categories_name='待归类';
        			break;
        ...
        echo '<br><b style="color:#ff0000">订货分类:'.$orders_products_categories_name.'</b>';
        ...
        if(empty($product['product_images'])){
            ...
            echo '<a class="field_content" rel="product_images" href="'.U('Product/Database/view', array('product_id'=>$product['product_id'], 'lang'=>$products_detail['language_code'], 'field'=>'product_images')).'">'.sizeof($images).'张</a>';            
        }

        if(!empty($product['additional_images'])){
            echo '<span title="'. $product['additional_images'] .'">有细节图</span>';
        }else{

        }
        ?>        
        </td>
        <td class="center"><?php if($product['retail_speciel_price']>0){ echo round($product['retail_speciel_price'],2); echo '<br>';} ?><?php echo round($product['speciel_price'], 2).'<br>'.($product['speciel_price']==0?'':'<del>').round($product['product_price'], 2).($product['speciel_price']==0?'':'<del>'); ?></td>
        <?php 
            if(preg_match('/\|\|\|/',$product['featured_category'])){
                $bigSize='大码';
            }else{
                $bigSize='标码';
            }
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <p class="help-block"><a href="__PUBLIC__/example/products.csv" target="__blank">csv模板下载</a></p>
                </div>      
            </div>
                   ...
                   $xuhao=0;
                    ...
                    switch ($entry['is_sale']) {
                     ...
                     if($entry['system_proupdate']=='True'){
                        $xuhao=$xuhao+1;
                        echo '<option value="'.$entry['site_id'].'">'.'(序号:'.$xuhao.'==ID:'.$entry['site_id'].'# 品牌:'.$entry['system_brand'].') '.$entry['site_name'].'('.$web_site.','.$beizhu.$zwb.$rxh.$entry['system_cms'].')'.'</option>';
                     }
