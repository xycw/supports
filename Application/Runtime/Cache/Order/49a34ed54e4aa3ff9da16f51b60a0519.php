...
<form action="<?php echo U('Order/PaymentModuleStatistics/index') ?>" method="POST">
  ...
  <div class="form-group">
    ...
    <select id="" name="site_type" onchange="" ondblclick="" class="form-control" ><option value="" >--不限--</option><?php  foreach($option_site_type as $key=>$val) { if(!empty($site_type_selected) && ($site_type_selected == $key || in_array($key,$site_type_selected))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
 ...
 foreach($site as $site_id=>$site_name){ ?>
  ...
  <div class="form-group">
    ...
    <select id="" name="payment_module[]" ondblclick="" onchange="" multiple="multiple" class="form-control" size="" ><?php  foreach($option_payment_module as $key=>$val) { if(!empty($payment_module_selected) && ($payment_module_selected == $key || in_array($key,$payment_module_selected))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
  ...
  <div class="form-group">
    ...
    <select id="" name="statistic_type" onchange="" ondblclick="" class="form-control" ><?php  foreach($option_statistic_type as $key=>$val) { if(!empty($statistic_type_selected) && ($statistic_type_selected == $key || in_array($key,$statistic_type_selected))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
...
<?php
 foreach($list as $date=>$data){ $n = sizeof($data); ?>

    <tr id="tr-<?php echo $date ?>">

        <td<?php if($n) echo ' rowspan="'.$n.'"' ?>><input type="checkbox" name="date[]" value="<?php echo $date ?>" /></td>

        <?php  if($n){ if($n==1 && is_null($data[0]['payment_module'])){ echo '<td colspan="10">此日期没有订单</td><td>'.$data[0]['update_time'].'</td></tr>'; }else{ $i = 0; foreach($data as $entry){ ?>
 ...
 if($n>1&&$i>0) echo '<tr>'; ?>
            ...
            <?php  switch ($entry['payment_module']) { case 'moneytransfers': $payment_module_name ='TW(独立站)'; break; case 'custom': $payment_module_name ='TW(商城站)'; break; case 'security_pingpong': $payment_module_name ='乒乓(独立站)'; break; case 'pingpong': case 'pingpong2f': $payment_module_name ='乒乓(商城站)'; break; case 'zdcheckout2f3d': $payment_module_name ='佐道2f3d'; break; case 'zdcheckout3f': $payment_module_name ='佐道3f'; break; case 'tpo': $payment_module_name ='中外宝(独立站)'; break; case 'mycheckout2f3d': $payment_module_name ='中外宝(商城站)'; break; case 'rxhpay_inline': $payment_module_name ='融信汇(独立站)'; break; case 'rxhpay': $payment_module_name ='融信汇(商城站)'; break; default: $payment_module_name=$entry['payment_module']; break; } ?>
