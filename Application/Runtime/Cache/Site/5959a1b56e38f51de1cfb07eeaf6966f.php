...
<div class="container">
<link rel="stylesheet" type="text/css" href="/supports/Public/Js/bootstrap-select/css/bootstrap-select.css" />
<script type="text/javascript" src="/supports/Public/Js/bootstrap-select/js/bootstrap-select.js"></script>
<h1>独立站.列表</h1>
...
<button class="btn btn-default" type="button" id="btn-multiple-down-inquery">咨询批量下载</button>
<button class="btn btn-default" type="button" id="btn-multiple-down-customer">客户数据批量下载</button>
<button class="btn btn-default" type="button" id="btn-multiple-down-cfg">配置批量下载</button>
<button class="btn btn-default" type="button" id="btn-multiple-replace-email">批量更换邮箱</button>
<?php }?>
<form id="form-search" class="advanced-search form-horizontal" action="<?php echo U('Site/Site/list') ?>" method="get">
    <div class="row">
        <label for="taobao-no" class="col-lg-1">商品数据</label>
        <div class="col-lg-2">
            <?php
 $sale = array( array('val' => -1, 'txt' => 'ALL', 'sel' => $_GET['is_sale'] == -1 ? 'selected="selected"' : ''), array('val' => 0, 'txt' => '零售', 'sel' => $_GET['is_sale'] == 0 ? 'selected="selected"' : ''), array('val' => 1, 'txt' => '批发', 'sel' => $_GET['is_sale'] == 1 ? 'selected="selected"' : ''), array('val' => 3, 'txt' => '帽子', 'sel' => $_GET['is_sale'] == 3 ? 'selected="selected"' : ''), array('val' => 4, 'txt' => '面罩', 'sel' => $_GET['is_sale'] == 4 ? 'selected="selected"' : ''), array('val' => 5, 'txt' => '袜子', 'sel' => $_GET['is_sale'] == 5 ? 'selected="selected"' : ''), array('val' => 6, 'txt' => 'anti-virus', 'sel' => $_GET['is_sale'] == 6 ? 'selected="selected"' : ''), array('val' => 7, 'txt' => '游戏手柄', 'sel' => $_GET['is_sale'] == 7 ? 'selected="selected"' : ''), array('val' => 8, 'txt' => '测试', 'sel' => $_GET['is_sale'] == 8 ? 'selected="selected"' : ''), array('val' => 9, 'txt' => 'Other', 'sel' => $_GET['is_sale'] == 9 ? 'selected="selected"' : ''), array('val' => 10, 'txt' => '定制', 'sel' => $_GET['is_sale'] == 10 ? 'selected="selected"' : ''), ); ?>
            <select name="is_sale" class="form-control">
                <?php
 foreach ($sale as $ts) { echo "<option value=" . $ts['val'] . " " . $ts['sel'] . ">" . $ts['txt'] . "</option>\n"; } ?>
            </select>
        </div>
...
<table class="table table-bordered mt5">
    ...
    <?php
 $n = 1; foreach ($site_list as $entry) { ?>
        <tr>
            <?php if(!in_array(session(C('USER_INFO').'.profile_id'), array(3,6))){?>
            <td>
                <?php ?>
                ...
                <ul>
                    ...
                    <li>
                        业务员:
						    ...
						    <?php
 if (isset($entry['user'][2])) { foreach ($entry['user'][2] as $user) { echo '<span style="color:#FF0000;font-weight:bold;">'.$user['chinese_name'] . '</span>'; } } ?>
						 客服昵称:
           ...
           <!--  <td id="remark<?php ?>"><?php ?> -->
            <td>
                	<?php  $styledomain=' '; $styleDshow = ' style="color:#ff0000"'; if (($entry['days_expired'] < 7) and ($entry['date_expired'] <> '0000-00-00')){ $styledomain= ' style="background-color:#ffff00;color:#ff0000;"'; } ?>
                <ul>
                    ...
                    <?php
 $is_sale_mark='xxx'; switch ($entry['is_sale']) { case '-1': $is_sale_mark = '不清缓存'; case '0': $is_sale_mark = '零售'; break; case '1': $is_sale_mark = '批发'; break; case '2': $is_sale_mark = 'B站'; break; case '3': $is_sale_mark = '帽子'; break; case '4': $is_sale_mark = '面罩'; break; case '5': $is_sale_mark = '袜子'; break; case '6': $is_sale_mark = 'anti-virus'; break; case '7': $is_sale_mark = '游戏手柄'; break; case '8': $is_sale_mark = '测试'; break; case '9': $is_sale_mark = 'Other'; break; case '10': $is_sale_mark = '定制'; break; default: break; } echo '<b>商品数据:</b><span style="color:#ff0000;font-weight:bold">'.$is_sale_mark.'</span>'; ?>
                	</li>
                	<hr>
 ...
 if (!empty($entry['email_data'])) { ?>
        ...
        <?php
 $email_data = json_decode($entry['email_data'], true); foreach ($email_data as $email_info) { ?>
                            <tr>
            ...
            <hr>
               <h3>网站权限</h3>
               <br>路径: 
            <?php  if(!empty($entry['system_weburl'])){ $my_weburl=$entry['site_index'].'/'.$entry['system_weburl']; ?>
