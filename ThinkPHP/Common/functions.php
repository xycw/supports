...
function send_mail($to, $name, $subject = '', $body = ''){
    ...
    $mail->Port = 587;
    $mail->Username = "xingchenwo2021@163.com";
    $mail->Password = "HCRVFOUDZZOSCEDY";
    $mail->setFrom("xingchenwo2021@163.com", "业务系统管理员"); // 发件人邮箱和名称
    $mail->Subject = $subject;
    $mail->Body = $body;
    $mail->isHTML(true); //是否启用html
    $mail->addAddress($to); //收件用户
    return $mail->Send() ? array('status' => 1) : array('status' => 0, 'info' => $mail->ErrorInfo);
}