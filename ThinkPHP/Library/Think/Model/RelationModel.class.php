        ...
        if(!empty($this->_link)) {
            foreach($this->_link as $key=>$val) {
                    ...
                    if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                        ...
                        switch($mappingType) {
                            case self::HAS_ONE:
	                            	if (is_array($mappingKey) && is_array($mappingFk)) {
	                            		foreach ($mappingKey as $k=>$_mappingKey){
	                            			$pk   =  $result[$_mappingKey];
	                            			$_mappingFk = $mappingFk[$k];
	                            			$mappingCondition .= " AND {$_mappingFk}='{$pk}'";
	                            		}
	                            	}else{
                            ...
                            switch($mappingType) {
                                case self::HAS_ONE:
                                    switch (strtoupper($opType)){
                                        ...
                                        $result   =  $model->add($mappingData,array('link'=>$name),true);
                                        break;
                                        case 'SAVE':    // 更新关联数据
                                ...
                                case self::HAS_MANY:
                                    switch (strtoupper($opType)){
                                        ...
                                        foreach ($mappingData as $val){
                                            $val[$mappingFk]    =   $pk;
                                            $result   =  $model->add($val,array('link'=>$name),true);
                                        }
                                        $model->commit();
                                        break;
                                        case 'SAVE' :   // 更新关联数据
